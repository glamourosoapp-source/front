...
- **Una nota se imprime una sola vez**: imprimir (listado o detalle) pasa por `POST /orders/print`, que marca `printedAt`/`printedBy`; el listado pinta esas filas de azul (`isRowHighlighted` de `DataTable`), les deshabilita el checkbox y el detalle deja el botón en "Ya impreso". Solo un rol admin (`usePermissions().isAdmin`) puede reimprimir, y el server también lo valida (409).
