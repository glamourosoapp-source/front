...
- Impresión: clases `.print-only` + `@media print` en `globals.css`; exports Excel/PDF paginan de a 200. La hoja imprimible **debe montarse por portal como hijo directo de `<body>`** (ver `OrderPrintSheet`): el `@media print` saca del flujo todo lo demás con `body > *:not(.print-only) { display: none }`; si se anida en el layout, el alto del dashboard vuelve a generar una página en blanco.
